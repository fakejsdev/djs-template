...
const discoverComponentHandlers = async () => {
  ...
  for (const file of handlerFiles) {
    try {
      ...
      const handleFunction = module[`handle${capitalize(componentType)}Interaction`];
      ...
      if (setupFunction && handleFunction) {
        ...
        Console.Warn(`Skipping ${componentType} - missing setup or handle function`);
      }
    } catch (error) {
...
const setupComponentHandlers = async () => {
  ...
  if (totalComponents > 0) {
    Console.Log(`🧩 Loaded ${totalComponents} component${totalComponents === 1 ? '' : 's'}`);
  }
};
