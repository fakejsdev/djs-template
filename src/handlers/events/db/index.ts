...
const setupDatabaseEventFiles = async () => {
  ...
  for (const file of eventFiles) {
    ...
    const { config, run }: DatabaseEventConfigWithRun = importedFile.default || importedFile;

    if (!config || !run) throw new Error('Database event file must export both config and run');

    const eventKey = config.name;

    if (events.has(eventKey)) throw new Error(`Duplicate database event ${config.name}`);

    events.set(eventKey, { config, run });
  }

  if (events.size > 0) {
    Console.Log(`💾 Loaded ${events.size} Database event${events.size === 1 ? '' : 's'}`);
  }
  return events;
};
